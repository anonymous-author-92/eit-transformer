...
class CrossEntropyLoss2d(_WeightedLoss):
    ...
    def forward(self, preds, targets, weights=None):
        ...
        if self.regularizer:
            ...
            if self.debug:
                print(f"weight \t {weights.size()}")
                print(f"L1 difference size \t {L1_diff.size()}")
...
class L2Loss2d(_WeightedLoss):
    ...
    def forward(self, preds, targets, weights=None):
        ...
        if weights is None and self.weighted:
            ...
            weights = inv_L2_norm/inv_L2_norm.sum()
        elif not self.weighted:
            weights = 1

        loss = self.beta*weights*((preds - targets).pow(2)).sum(dim=(1, 2, 3))/target_norm
