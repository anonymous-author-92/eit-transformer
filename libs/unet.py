...
class Unet(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.up7 = nn.ConvTranspose2d(512, 256, 2, stride=2, output_padding=1)
        self.conv7 = DoubleConv(512, 256)
        self.up8 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        self.conv8 = DoubleConv(256, 128)
        self.up9 = nn.ConvTranspose2d(128, 64, 2, stride=2, output_padding=1)
    ...
    def forward(self, x, xp=None, pos=None, grid=None):  # 101x101x20
        ...
        c1 = self.conv1(x)                         # 101x101x64
        p1 = self.pool1(c1)                        # 50x50x64
        c2 = self.conv2(p1)                        # 50x50x128
        p2 = self.pool2(c2)                        # 25x25x128
        c3 = self.conv3(p2)                        # 25x25x256
        p3 = self.pool3(c3)                        # 12x12x256
        c4 = self.conv4(p3)                        # 12x12x512
        p4 = self.pool4(c4)                        # 6x6x512
        c5 = self.conv5(p4)                        # 6x6x1024
        up_6 = self.up6(c5)                       # 12x12x512
        merge6 = torch.cat([up_6, c4], dim=1)    # 12x12x1024
        c6 = self.conv6(merge6)                    # 12x12x512
        up_7 = self.up7(c6)                        # 25x25x256
        merge7 = torch.cat([up_7, c3], dim=1)    # 25x25x512
        c7 = self.conv7(merge7)                    # 25x25x256
        up_8 = self.up8(c7)                        # 50x50x128
        merge8 = torch.cat([up_8, c2], dim=1)    # 50x50x256
        c8 = self.conv8(merge8)                    # 50x50x128
        up_9 = self.up9(c8)                        # 101x101x64
        merge9 = torch.cat([up_9, c1], dim=1)        # 101x101x128
        c9 = self.conv9(merge9)                    # 101x101x64
        c10 = self.conv10(c9)                      # 101x101x1
        # out = nn.Sigmoid()(c10)
        # out = nn.Tanh()(c10)
        # return c10
        return dict(preds=c10.permute(0, 2, 3, 1),
                    preds_latent=c6.permute(0, 2, 3, 1))
...
class BasicTransDecoderBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        x1c = self.conv_ch(x1)
        res = F.interpolate(x1c, size=x2.shape[-2:], mode='bilinear', align_corners=True)
...
class LinearAttention(nn.Module):
    ...
    def __init__(self, dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, bias=False):
        ...
        self.to_qkv = depthwise_separable_conv(dim, self.inner_dim*3, bias=self.bias)
        self.to_out = depthwise_separable_conv(self.inner_dim, dim, bias=self.bias)
...
class LinearAttentionDecoder(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, bias=False):
        ...
        self.to_kv = depthwise_separable_conv(in_dim, self.inner_dim*2, bias=bias)
        self.to_q = depthwise_separable_conv(out_dim, self.inner_dim, bias=bias)
        self.to_out = depthwise_separable_conv(self.inner_dim, out_dim, bias=bias)
...
class UTransformer(nn.Module):
    ...
    def __init__(self, in_chan, base_chan, 
        ...
        self.up4 = up_block(2*base_chan, base_chan,
                            scale=(2, 2), num_block=2, debug=debug)
        self.inc = nn.Sequential(*self.inc)

        self.down1 = down_block_trans(base_chan, 2*base_chan, num_block=num_blocks[-4], bottleneck=bottleneck, maxpool=maxpool, heads=num_heads[-4],
                                        dim_head=2*base_chan//num_heads[-4], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, bias=bias)
        self.up3 = up_block_trans(4*base_chan, 2*base_chan, num_block=0, bottleneck=bottleneck,
                                    heads=num_heads[-3], dim_head=2*base_chan//num_heads[-3], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, 
                                    bias=bias, debug=debug)

        self.down2 = down_block_trans(2*base_chan, 4*base_chan, num_block=num_blocks[-3], bottleneck=bottleneck, maxpool=maxpool, heads=num_heads[-3],
                                        dim_head=4*base_chan//num_heads[-3], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, bias=bias)
        self.up2 = up_block_trans(8*base_chan, 4*base_chan, num_block=0, bottleneck=bottleneck,
                                    heads=num_heads[-2], dim_head=4*base_chan//num_heads[-2], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, bias=bias, debug=debug)

        self.down3 = down_block_trans(4*base_chan, 8*base_chan, num_block=num_blocks[-2], bottleneck=bottleneck, maxpool=maxpool, heads=num_heads[-2],
                                        dim_head=8*base_chan//num_heads[-2], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, bias=bias)
        self.up1 = up_block_trans(16*base_chan, 8*base_chan, 
                                    num_block=0, bottleneck=bottleneck,
                                    heads=num_heads[-1], 
                                    dim_head=8*base_chan//num_heads[-1], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, 
                                    projection=projection, rel_pos=rel_pos, bias=bias,
                                    debug=debug)

        self.down4 = down_block_trans(8*base_chan, 16*base_chan, num_block=num_blocks[-1], bottleneck=bottleneck, maxpool=maxpool, heads=num_heads[-1],
                                        dim_head=16*base_chan//num_heads[-1], attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, bias=bias)

        self.outc = nn.Conv2d(base_chan, num_classes, kernel_size=1, bias=True)

        if aux_loss:
            self.out1 = nn.Conv2d(8*base_chan, num_classes,
                                  kernel_size=1, bias=True)
            self.out2 = nn.Conv2d(4*base_chan, num_classes,
                                  kernel_size=1, bias=True)
            self.out3 = nn.Conv2d(2*base_chan, num_classes,
                                  kernel_size=1, bias=True)
        self.batchnorm = batchnorm
...
if __name__ == '__main__':
    ...
    summary(model, input_size=[(16, 201, 201, 3), (16, 201, 201, 6)])